...
export default function Features() {
  ...
  useEffect(() => {
    const fetchFeaturesArticles = async () => {
      setLoading(true);
      setError(null);
      try {
        const response = await axios.get('/api/articles', {
          params: { category: 'features', page: currentPage }
        });
        setArticles(response.data.data || []);
        setTotalPages(response.data.last_page || 1);
      } catch (err) {
        console.error('Error fetching features articles:', err);
        setError('Failed to load features articles. Please try again later.');
        setArticles([]);
        setTotalPages(1);
      } finally {
        setLoading(false);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="container mx-auto grow">
        <div className="bg-news-bg bg-cover bg-center h-20" style={{
                  backgroundImage: `linear-gradient(to right, #FFA500 20%, rgba(255,165,0,0.2)), url('/src/assets/images/bg.jpg' )`
                }}>
          <h1 className="text-5xl font-bold text-white justify-center flex items-center h-full md-8">FEATURES</h1>
        </div>
        {loading ? (
