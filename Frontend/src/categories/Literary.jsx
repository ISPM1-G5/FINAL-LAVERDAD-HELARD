...
export default function Literary() {
  ...
  useEffect(() => {
    const fetchLiteraryArticles = async () => {
      setLoading(true);
      setError(null);
      try {
        const response = await axios.get('/api/articles', {
          params: { category: 'literary', page: currentPage }
        });
        setArticles(response.data.data || []);
        setTotalPages(response.data.last_page || 1);
      } catch (err) {
        console.error('Error fetching literary articles:', err);
        setError('Failed to load literary articles. Please try again later.');
        setArticles([]);
        setTotalPages(1);
      } finally {
        setLoading(false);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="container mx-auto grow">
        <div className="bg-news-bg bg-cover bg-center h-20" style={{
                  backgroundImage: `linear-gradient(to right, #228B22 20%, rgba(34,139,34,0.2)), url('/src/assets/images/bg.jpg' )`
                }}>
                  <h1 className="text-5xl font-bold text-white justify-center flex items-center h-full md-8">LITERARY</h1>
                </div>
