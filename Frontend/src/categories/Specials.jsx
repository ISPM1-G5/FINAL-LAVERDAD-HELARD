...
export default function Specials() {
  ...
  useEffect(() => {
    const fetchSpecialsArticles = async () => {
      setLoading(true);
      setError(null);
      try {
        const response = await axios.get('/api/articles', {
          params: { category: 'specials', page: currentPage }
        });
        setArticles(response.data.data || []);
        setTotalPages(response.data.last_page || 1);
      } catch (err) {
        console.error('Error fetching specials articles:', err);
        setError('Failed to load specials articles. Please try again later.');
        setArticles([]);
        setTotalPages(1);
      } finally {
        setLoading(false);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="container mx-auto grow">
        <div className="bg-news-bg bg-cover bg-center h-20" style={{
          backgroundImage: `linear-gradient(to right, #00008B 20%, rgba(0,0,139,0.2)), url('/src/assets/images/bg.jpg' )`
        }}>
